import { initOpenCascade } from './OpenCascadeHelper';
import type { OpenCascadeInstance } from 'opencascade.js/dist/opencascade.full';

/**
 * Parametric Modeler using OpenCascade.js
 * Reference: opencascade.js-examples/src/demos/bottle - basic/library.js
 */
export class ParametricModeler {
    private static oc: OpenCascadeInstance | null = null;

    /**
     * Initialize OpenCascade and cache the instance
     */
    static async initialize(): Promise<OpenCascadeInstance> {
        if (!this.oc) {
    ...
    /**
     * Create a parametric bottle shape.
     * Ported from opencascade.js-examples/src/demos/bottle - basic/library.js
     */
    static async makeBottle(myWidth: number, myHeight: number, myThickness: number) {
        const openCascade = await this.initialize() as any;
        ...
        let myBody: any = new openCascade.BRepPrimAPI_MakePrism_1(myFaceProfile.Face(), aPrismVec, false, true);
