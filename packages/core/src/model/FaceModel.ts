...
export interface FaceGraphItem {
    ...
    path: THREE.Vector3[];
    /** Per-vertex UVs (0..1) for tiles; optional, generated by pattern rebuild */
    uvs?: THREE.Vector2[];
...
export class FaceModel extends BaseModel {
    ...
    getGraphData(): FaceGraphData {
        ...
        for (const region of regions) {
            ...
            // Tile paths → use the material returned by rebuild()
            for (let i = 0; i < result.tilePaths.length; i++) {
                items.push({
                    type: 'tile',
                    path: result.tilePaths[i],
                    uvs: result.tileUVs[i],
                    material: result.material,
                });
            }

            // Gap paths → use the gap material returned by rebuild() (fallback to tile material)
            const gapMat = result.gapMaterial ?? result.material;
